...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    if [[ "$distro" == "arch" || "$id_like" == "arch" ]]; then
       ...
       sudo apt update && sudo apt install -y unace unrar zip unzip p7zip-full p7zip-rar sharutils rar uudeview mpack arj cabextract file-roller device-tree-compiler liblzma-dev python-pip brotli liblz4-tool gawk aria2
    fi
